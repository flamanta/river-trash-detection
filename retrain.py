  ...
  """
    with tf.Graph().as_default() as graph:
        ...
        with gfile.FastGFile(model_filename, "rb") as f:
            ...
            bottleneck_tensor, jpeg_data_tensor, resized_input_tensor = tf.import_graph_def(
                graph_def,
                name="",
                return_elements=[
...
def add_input_distortions(
  ...
  """
    ...
    resize_scale_value = tf.random.uniform(
    ...
    precropped_image = tf.image.resize(
        decoded_image_4d, precrop_shape_as_int, method=tf.image.ResizeMethod.BILINEAR
    )
    ...
    brightness_value = tf.random.uniform(
